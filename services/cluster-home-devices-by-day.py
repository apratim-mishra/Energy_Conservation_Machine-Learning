...
if __name__ == "__main__":
    ...
    rdd_for_day.persist(pyspark.StorageLevel.DISK_ONLY)  # needed as mongodb connector has bugs.
    #import rpdb; rpdb.set_trace()
    ...
    try:
        date_obj = datetime.datetime.strptime(input_date,
                                                  "%Y-%m-%d")
    except ValueError:
        date_obj = datetime.datetime.strptime(input_date,
                                                          "%Y-%m-%d")
    weekdays_to_process = [date_obj.weekday()]
    print("weekday:" + str(date_obj.weekday()))
    for weekday in weekdays_to_process :  # [0 to 6]
        for hour in range(0, 24):
            ...
            # predict_data = data.take(1)[0]
            # print("predict cluster number:" + str(optimal_model.predict(predict_data)))
            # print("num_clusters:" + str(optimal_clusters) + " Total Cost: " + str(cost))
            # print("Final centers: " + str(optimal_model.clusterCenters))
            # print("predict cluster: " + str(optimal_model.predict(data.collect()[0])))  # check cluster of first element
